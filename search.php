...
<div class="main_column column" id="main_column">
    ...
    if($query == "")
		 ...
		 //if query contains an underscore, assume user is searching fr usernames
	    if($type == "username")
		 	$userReturnedQuery = mysqli_query($con, "SELECT * FROM users WHERE username LIKE '$query%' AND user_closed='no' LIMIT 8");

		 //if there are two words, assume first and last name 
		else {
	    ...
	    while($row = mysqli_fetch_array($userReturnedQuery)) {
	        ...
	        if($user['username'] != $row['username']) {
	        	...
	        	//Generate button based on friends status
	        	if($user_obj->isFriend($row['username']))
	        		$button = "<input type='submit' name='" . $row['username'] . "' class='danger' value='Remove Friend'>";
	        	else if($user_obj->didReceiveRequest($row['username']))
	        		$button = "<input type='submit' name='" . $row['username'] . "' class='warning' value='Accept Request'>";
